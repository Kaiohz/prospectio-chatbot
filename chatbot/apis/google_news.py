...
class GoogleNewsApi:
    async def get_top_headlines_sources(country: str) -> SourcesResponse:
        ...
        sources_response: SourcesResponse = SourcesResponse(
            **await Api.get(headlines_sources_endpoint, params=params)
        )
        return sources_response

    async def get_top_headlines(sources: List[str]) -> TopHeadlines:
        params = {"apiKey": api_key, "sources": sources}
        top_headlines_response = TopHeadlines(
            **await Api.get(top_headlines_endpoint, params=params)
        )
    ...
    async def get_everything(topic: str) -> ArticlesResponse:
        ...
        articles_response: ArticlesResponse = ArticlesResponse(
            **await Api.get(everything_endpoint, params=params)
        )
