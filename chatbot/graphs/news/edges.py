...
class NewsConditionalEdges:
    ...
    def __init__(self, graph_params: GraphParams):
        self.HeadlinesOrSpecificChain = HeadlinesOrSpecificChain(
            model=graph_params.model,
            temperature=graph_params.temperature,
            prompt=self.PromptLoader.load_prompt("HeadlinesOrSpecific"),
        )

    async def headlines_or_specific(self, state) -> str:
        question = state["question"]
        try:
            choice_model: ChoiceModel = ChoiceModel(
                **await self.HeadlinesOrSpecificChain.chain.ainvoke(
                    {"question": question}
                )
            )
        except:
