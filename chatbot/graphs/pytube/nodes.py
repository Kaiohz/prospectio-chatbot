...
class PytubeNodes:
    ...
    def __init__(self, graph_params: GraphParams):
        self.SummarizerChain = SummarizerChain(
            model=graph_params.model,
            temperature=graph_params.temperature,
            prompt=self.PromptLoader.load_prompt("Summarizer"),
        )
    ...
    async def summarizer(self, state) -> PytubeState:
        ...
        if transcript:
            summaries = await self.SummarizerChain.chain.ainvoke(
    ...
    async def generate(self, state) -> PytubeState:
        ...
        final_answer = await self.GenerateChain.chain.ainvoke(
